...
public class ScrambleString {
    ...
    static char[] cs1;  // s1
    static char[] cs2;  // s2
    static byte[][][] grid; // 网格搜索
    static int[] arr = new int[26]; // 26个字符

    public static boolean isScramble(String s1, String s2) {
        if (s1.length() == 1) {
            return s1.charAt(0) == s2.charAt(0); // 长度为1，判断相等
        }
        cs1 = s1.toCharArray(); // 转化数组
        cs2 = s2.toCharArray(); // 转化数组
    ...
    // abc acb
    // abc abx
    public static boolean isCharCountSame(int i1, int i2, int length) {
    ...
    public static void main(String[] args) {
        System.out.println(isScramble("abcd", "cabd"));
    }
